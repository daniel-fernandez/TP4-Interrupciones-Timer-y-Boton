...
GPIO_TypeDef* leds_port_tira[] = { GPIOD, GPIOD, GPIOD, GPIOD, GPIOD, GPIOD, GPIOD, GPIOD };
/* Leds disponibles */
const uint16_t leds[] = { LED_V, LED_R, LED_N, LED_A };
const uint16_t leds_tira[] = {LED_0, LED_1, LED_2, LED_3, LED_4, LED_5, LED_6, LED_7};

uint32_t* const leds_pwm[] = { &TIM4->CCR1, &TIM4->CCR3, &TIM4->CCR2, &TIM4->CCR4 };
//punteros a variables uint32, es una estructura constante, leds_pwm es un arreglo constante de tipo uint32,
//y apuntamos a variables de tipo uint32.
...
//	GPIO_SetBits(leds_port[led], leds[led]);
	GPIO_SetBits(leds_port_tira[led], leds_tira[led]);
}

void led_off(uint8_t led) {
//	GPIO_ResetBits(leds_port[led], leds[led]);
	GPIO_ResetBits(leds_port_tira[led], leds_tira[led]);
}
...
float bsp_get_acc(char eje){
	...
	case 'x':
	case 'X':
		return LIS3DSH_Get_X_Out(LIS3DSH_Sense_2g);   //sense 2g es la sensibilidad que le damos al acelerometro
	case 'y':
	case 'Y':
